...
static String textFull;
	...
	public static WikipediaDocument Initalizer(String text,WikipediaDocument mydoc)
	{
		...
		for(int i=0;i<lines.length;i++)
		{
			...
			//textFull=textFull+lines[i]+"\n";
			//textFull=textFull.concat(lines[i]).concat("\n");
			textFull_tmp=null;
		...
		while(textFull.contains("[[Image:"))
		{
			...
			//textFull=textFull.substring(0,index11)+textFull.substring(index13+2);
			
			//textFull=textFull.substring(0,index11).concat(textFull.substring(index13+2));
		...
		while(textFull.contains("[[")&&textFull.contains("]]"))
		{
			...
			//textFull=textFull.substring(0,index1)+textandlink[0]+textFull.substring(index2+2);
			/*textFull=textFull.substring(0,index1).concat(textandlink[0]).concat(textFull.substring(index2+2));*/
		...
		while(textFull.contains("[http"))
		{
			...
			if((index2<index3)||index3==-1) /*textFull=textFull.substring(0,index1)+textandlink[0]+textFull.substring(index2+1);*/ /*textFull=textFull.substring(0,index1).concat(textandlink[0]).concat(textFull.substring(index2+2));*/
			{
				textFull_tmp=null;
				textFull_tmp=textFull.substring(0,index1).concat(textandlink[0]).concat(textFull.substring(index2+2));
				textFull=textFull_tmp;
			}	
			else /*textFull=textFull.substring(0,index1).concat(textandlink[0]).concat(textFull.substring(index3));*/
			{
				textFull_tmp=null;
	...
	public static String parseTagFormatting(String text) {
		...
		{	
		/*Pattern tag1=Pattern.compile("[\\<][a-zA-z 0-9 \\s \\= \\' \\\" \\& \\- \\! \\: \\] \\[ \\*]+[\\/][\\>]");
		Pattern tag2=Pattern.compile("[\\<][a-zA-z 0-9 \\s \\= \\' \\\" \\& \\- \\! \\: \\] \\[ \\*]+[\\>]");
		Pattern tag3=Pattern.compile("[\\<][\\/][a-zA-z 0-9 \\s \\= \\' \\\" \\& \\- \\! \\: \\] \\[ \\*]+[\\>]");
		Pattern tag4=Pattern.compile("[\\&][l][t][\\;][a-zA-z 0-9 \\s \\= \\' \\/ \\\" \\& \\- \\! \\: \\] \\[ \\*]*[g][t][\\;]");//--ask niranjan
*/		
		//Pattern tag4=Pattern.compile("[\\&][l][t][\\;][.]*[g][t][\\;]");
	...
	public static String parseTemplates(String text) {
		//Pattern TempPat=Pattern.compile("[\\{]{2}[a-zA-Z\\s\\d\\.\\|\\=\\_\\[\\]\\,\\&\\;\\-\\/\\(\\)\\!\\-\\:\\'\\? \0\\%  \\+ \\< \\> \\� \\� \\w]+[\\}]{2}");
	...
	public static String[] parseLinks(String text) {
		...
		{			
			...
			if(index4>0)
			{
				//par_txt=par_txt+text.substring(index4+1, index3);
				par_txt=par_txt.concat(text.substring(index4+1, index3));
			}
			
			if((index3<tex_len-1)&&index3>0)
			{
				//par_txt=par_txt+text.substring(index3+2,tex_len);
		...
		else if(text.contains("[http"))
		{
			...
			if(index4>0)
			{
			if(index3>0) /*par_txt=par_txt+text.substring(index4+1, index3);*/ par_txt=par_txt.concat(text.substring(index4+1, index3));
			else		/*par_txt=par_txt+text.substring(index4+1);*/ par_txt=par_txt.concat(text.substring(index4+1));
			}
			
			if((index3<tex_len-1)&&index3>0)
			{
				/*par_txt=par_txt+text.substring(index3+1,tex_len);*/ par_txt=par_txt.concat(text.substring(index3+1,tex_len));
			}
		}
		...
		else if(m7.find())
		{
			...
			if(text.contains("|]]"))
				/*par_txt=par_txt+text.substring(index6+1,index5);*/
				par_txt=par_txt.concat(text.substring(index6+1,index5));
			else if(index5>0)
				/*par_txt=par_txt+text.substring(index4+1,index5);*/
		...
		else if(m4.find())
		{
			...
			if(text.contains("|")&&!text.contains("|]]"))
				/*par_txt=par_txt+text.substring(index2+1,index3);*/
				par_txt=par_txt.concat(text.substring(index2+1,index3));
			else if(index6>0)
			{	
				if(index2>0) /*par_txt=par_txt+text.substring(index1+2,index2);*/ par_txt=par_txt.concat(text.substring(index1+2,index2));
				else /*par_txt=par_txt+text.substring(index1+2,index3);*/ par_txt=par_txt.concat(text.substring(index1+2,index3));
			}
			else if(index5>0)
				/*par_txt=par_txt+text.substring(index4+1,index5);*/
				par_txt=par_txt.concat(text.substring(index4+1,index5));
			else if(text.contains("|]]"))
				/*par_txt=par_txt+text.substring(index4+1,index2);*/
				par_txt=par_txt.concat(text.substring(index4+1,index2));
			else
				/*par_txt=par_txt+text.substring(index1+2,index3);*/
		...
		else if(m5.find())
		{
			...
			if(text.contains("|]]"))
				/*par_txt=par_txt+text.substring(text.indexOf(":")+1,index2);*/
		...
		else if(m6.find()||text.contains("simple:"))
		{
			...
			/*par_txt=text.replaceAll("[\\[]{2}","");
			par_txt=par_txt.replaceAll("[\\]]{2}","");
			par_txt=par_txt.replaceAll("\\|","");	*/
		}
		...
		else if(m3.find())
		{
			...
			if(index4>0)
			{	
				/*par_txt=par_txt+text.substring(index1+2,index4);*/
				/*par_txt=par_txt.concat(text.substring(index1+2,index4));*/
				partxt_temp=null;
				partxt_temp=par_txt.concat(text.substring(index1+2,index4));
				par_txt=partxt_temp;
			}
			else if(index5>0)
			{	/*par_txt=par_txt+text.substring(index1+2,index5);*/
				/*par_txt=par_txt.concat(text.substring(index1+2,index5));*/
				partxt_temp=null;
		...
		if(par_txt.length()>0)
		{
			...
			par_txt=partxt_temp;
			//par_txt = par_txt.replaceAll("<[A-Z a-z \\/]+>","");
		}	
		
		
		/*Pattern p1=Pattern.compile("[\\[]{2}[A-Z a-z \\s \\d \\- ' \" \\.,]+[\\|][A-Z a-z \\s \\d \\- ' \" \\.,]+[\\]]{2}");	
		...
		Pattern p2=Pattern.compile("[\\[]{2}[A-Z a-z \\s \\d \\- ' \\.,]+[\\]]{2}");	
		Matcher m2=p2.matcher(text);
		...
		Pattern p3=Pattern.compile("[\\[]{2}[A-Z a-z 0-9 ' \\.]+[\\(\\,][A-Z a-z \\) 0-9]+[\\|][A-Z a-z 0-9 ' \\.,]*[\\]]{2}");	
		Matcher m3=p3.matcher(text);
		...
		if(m3.find())
		{
			...
			link=text.substring(index1+2,index2).replaceAll(" ", "_");
		}
		...
		if(m4.find())
		{
			...
			if(index5>0)
				...
				par_txt=par_txt+text.substring(index1+2,index2);
			else if(index2>0)
				par_txt=par_txt+text.substring(index4+1,index2);
			else
				par_txt=par_txt+text.substring(index4+1,index3);
		...
		if(m5.find())
		{
			...
			p5=Pattern.compile("[A-Z a-z]+[:][A-Z a-z]+[:][.]*");	
			
			m5=p5.matcher(text);
			
			if(m5.find())
			{	
				par_txt=par_txt.substring(par_txt.indexOf(":")+1);
			}
		}
		...
		if(text.contains("[http"))
		{
			...
			if(index4>0)
			{
			if(index3>0) par_txt=par_txt+text.substring(index4+1, index3);
			else		par_txt=par_txt+text.substring(index4+1);
			}
			
			if(index3<tex_len-1)
			{
				par_txt=par_txt+text.substring(index3+1,tex_len);
			}
		}
